...
def predict():
    ...
    raw_customer = {
        ...
        "loan_int_rate": float(data.get("loan_int_rate", 10)),
    ...
    # --------------------------------------------------
    xgb_prob = float(xgb_model.predict_proba(df_final)[0][1])
    ...
    # 6. Decision Logic
    # --------------------------------------------------
    eligible = xgb_prob < 0.5

    score = int((1 - xgb_prob) * 550 + 300)
    ...
    response = {
        ...
        "score": int(score),
        "risk": risk,
        "confidence": float(round((1 - xgb_prob) * 100, 2)),
